...
## 2026-09-03 — The press: Pixie Dust Industries, three drums, and the site goes light (32 skills)

The morning's rebrand was the wrong answer to the right diagnosis. It found the real fault — every accent on the site was an unmodified Tailwind default, and Space Grotesk is the face that signals "designed" without doing any designing — and then answered it with another dark palette, which is a look you can generate. That is the one thing a product about doing the work properly should not be. This replaces it with the direction Niel had been developing with his designers: **Risograph**.

- **The house is Pixie Dust Industries and the product is the Product OS.** The name on the door and the thing it ships. "Industries" rather than "Services" because it buys the whole print-shop vocabulary — drums, passes, plates, press checks — and because a service does not have drums. Nothing addressable moved: the plugin is still `oolio-pm`, the repository is still `niel-cody/oolio-product-os`, and every install keeps working.
- **Three ink drums plus a black drum, on uncoated stock.** Fluorescent Pink `#FF48B0`, Blue `#3D5588`, Sun Yellow `#FFE800`, Riso Black `#231F20`, printed on `#E4E2DB`. Three is the practical ceiling before the paper buckles, and that ceiling is the reason the look holds together rather than a restriction to work around. The overprints are the rest of the palette and the press produces them, not us.
- **The site is light, and that is physics rather than preference.** Riso ink is translucent and needs a pale sheet to sit under it; the process cannot print on dark stock. Everything went with it: the landing page, the map, the skills, the changelog, the systems map and the Flightdeck.
- **The third drum is the argument.** Sun Yellow is reserved for the human sign-off gates and nothing else. It is the smallest ink budget on the sheet and the only warm thing on it, so "a person signs off anything that counts" becomes the loudest mark on the map — which is what the page has claimed in words all along. At 1.04:1 on the stock it can never be a line or a letter, so a gate is a yellow ground with black type on it, which measures 13:1, the highest contrast anywhere on the site.
- **The four moves, and the rules that stop them becoming a filter.** Grain is one fixed multiply pass at 19% over the whole page, so the sheet stays still while the ink moves. Overprint is `mix-blend-mode: multiply` with an isolated parent. Halftone is one screen angle per drum, 15°, 75° and 45°, because two drums at the same angle produce moiré. Misregistration is two ghost plates behind the solid, **once per screen, display type only**. Texture never touches anything functional: a command block gets a hard pink shadow and nothing else, because a shifted plate over a URL is a command somebody mistypes.
- **The star field is gone.** A 1600×900 animated constellation canvas was the most generated-looking thing on the page, and it was the same code that had leaked all thirty-two skill names into the public payload as React keys. The hero now carries a three-drum halftone plate: drawn rather than data, so there is nothing in it that can leak, and `getSky()` is no longer called at all.
- **Syne, Archivo and DM Mono.** Display, interface, machine. Syne was drawn for a French art centre rather than a startup and reads as an art press; Archivo does nothing interesting on purpose so Syne can; DM Mono is a typewriter rather than a terminal and carries every ink code, screen angle, count and slash command.
- **The map is a printed diagram now.** Squared keylined plates instead of rounded cards with a glow, an ink spine per node type, Sun Yellow gate discs with black ticks, and a screened ground. A press does not glow, so every glow filter came out.
- **The link preview screens for real.** Satori resolves no CSS masks, blend modes or repeating backgrounds, so the halftone is laid out there as individual dots on a fixed pitch, falling off from the top right. It is the one part of the press language that renderer can reproduce honestly.
- **The drift guard now knows two retired palettes**: the dark one from this morning and the Tailwind defaults that caused all of this. `npm --prefix site run check` still fails on either, or on a hand-edited `brand.css`.
- **What this costs, written down before anyone rediscovers it:** the site can never go dark while it is Riso; fluorescent pink can never carry a word at any size; misregistration is capped at one per screen. All three are in [brand/README.md](brand/README.md) under "Where Riso stops".

## 2026-09-03 — A brand, written down and generated (32 skills)

The Product OS had a visual design and no brand. That is not the same thing. There was no mark, no favicon beyond Next's default, nothing that said what the identity meant, and the palette was Tailwind's defaults: `teal-400`, `violet-400`, `amber-500`, `emerald-400`. It looked like every other developer tool because, measured in OKLCH, it was every other developer tool. The whole system now lives in [`brand/`](brand/), and the site is generated from it.

- **The mark is the Gate.** A ring with a break in it, and a bead of amber standing in the break. The ring is the loop, the break is where the loop does not close on its own, and the bead is the person who closes it. It is the team's own belief drawn in one stroke and one dot, it works at sixteen pixels, and it is also an O. Construction is exact and recorded: r=11 on a 32 grid, the ring swept 294° from −12°, the break 66° wide centred on −45°.
- **The palette had two collisions, and they were not cosmetic.** `--human` and `--loop` were 11° apart in hue, and `--orch` and `--output` were 19°. Six meanings, four distinguishable colours, so a gate on the map and a learning loop were telling a reader two different things in the same gold. The six are now spread 43° to 90° apart, matched in perceived lightness, and every one clears 6.5:1 on the ground. The loop moved from gold to rose, which is the largest visible change and the one with the clearest reason.
- **The signature colour is the amber of a review gate.** It used to be teal. Amber means a person decides here, so it is spent on the gates, on the skills a person runs, and on the primary call to action, and on nothing else. That rule is the brand: the product's argument is that the loop closes on a person, and the identity says so before a word is read.
- **Three typefaces, each allowed to say a different kind of thing.** Instrument Serif for the argument, Inter for the interface, JetBrains Mono for what the machine counted. That division is the positioning set in type: a written argument that a machine executes. Space Grotesk is retired; it was doing the job of looking distinctive without being distinctive, and it was the weaker face at the 12 and 13px where most of this interface lives.
- **The tokens are the source and the stylesheet is generated from them**, the same rule the site already follows for the skills. `brand/tokens/brand.tokens.json` builds `site/app/brand.css`, and `npm --prefix site run check` fails if the two have drifted **or if any retired colour is still sitting in the site**. A stray old value never looks broken, which is exactly why it needs a test: it looks like a slightly different grey, and it survives for quarters.
- **Paper mode, for documents.** Confluence, a printed one-pager, a light deck. The same nine ground names and seven accents at document values, each darkened until it clears 4.5:1 on white. A separate token set rather than a `prefers-color-scheme` query, because a light Confluence page and a dark product surface are two audiences, not one reader's preference.
- **The systems map stops contradicting itself.** A badge reading `AMBER GATE` was drawn in violet, because it took its band's colour. It is amber now.
- **A real favicon and a real link preview.** The tab icon is the Gate rather than Next's default triangle, and the Open Graph card carries the mark, the line in Instrument Serif and the counts in mono. The card is the single most-seen surface the brand has, because the way an internal tool spreads is somebody pasting the URL into Slack.
- **The Flightdeck's eight domain hues were two pairs of near-duplicates**, 14° and 19° apart. Redistributed to a minimum 34° separation and one lightness register, so the legend is decodable rather than nearly decodable.
- **Found while proving it in a browser, not by reading the CSS:** Chrome rounds glyph advances to whole pixels while hinting, and against Instrument Serif's negative tracking the rounding accumulated unevenly, so "Oolio Product OS" rendered as "Prod uct OS" with a hole mid-word and the word space squeezed shut. It was there at 17, 18, 20, 22 and 24px. `text-rendering: geometricPrecision` on `.display` and `.wordmark` fixes it at all five, and removing the line brings it back.

## 2026-09-01 — Three roles, and access stops living in an environment variable (32 skills)

Who could use the site was `FLIGHTDECK_ALLOWED_EMAILS`, a comma-separated list in Vercel. Granting somebody access meant editing an environment variable and redeploying, and there was nowhere to record what they were allowed to do once in: you were on the list or you were not. Access now lives in a `members` table in Supabase, each person carries a role, and it is managed on a page rather than in a deploy.

- **Viewer, user, admin.** A viewer reads the reference surfaces: the map, the skills, the changelog, the systems. A user adds Flightdeck, which is somebody's actual diary, and in a later pass their own integrations. An admin adds managing the list. The roles are a Postgres enum declared weakest-first, so `role >= 'user'` is the ladder and cannot be written backwards.
- **`lib/routes.ts` is the only table of who can reach what.** The gate, the navigation and the pages all read it, so a link can never appear that bounces — a viewer simply does not see Flightdeck rather than seeing it and being refused, which reads as a broken site rather than as a door that is not theirs.
- **It still fails closed, which was the one property of the env allowlist worth keeping.** Access is the presence of a row. No row, an empty table, an unreachable database, a failed query and a role the code does not recognise all come out as no access. The alternative — an outage that opens the door — is the failure mode this shape exists to prevent.
- **Three things the database enforces rather than the app.** RLS lets you read your own row and admins read everyone, and only admins may write. A deferred constraint trigger refuses the update or delete that would leave no admin at all, because demoting the last one is a single click with no undo from inside the app. And `last_seen_at` is written by a definer function rather than a policy, because RLS cannot restrict which columns a policy covers, so an "update your own row" policy would have let anybody set their own role.
- **Signing in knows where to send you.** A viewer following a Flightdeck link used to be a plausible way to be told no by the site that had just let them in; the callback now lands each role somewhere it can actually reach. The gate also separates "not a member" from "not this role": the first destroys the session, the second leaves it alone, because signing somebody out of the pages they can use is a punishment for a mistyped URL.
- **Verified against the database, not just the build.** Acting as each role through RLS: a viewer sees one row and cannot promote themselves or delete an admin, a user sees one row, a non-member sees none and resolves to no role, an admin sees everyone. Supabase's own linter flagged three things in the first cut — a mutable `search_path`, and two trigger functions reachable as public RPC endpoints — all fixed.
- `FLIGHTDECK_ALLOWED_EMAILS` and `FLIGHTDECK_ALLOWED_DOMAINS` are retired and can be deleted from Vercel. Nothing reads them.

**Found on the way, and not fixed here:** this Supabase project also carries the Oolio Awards schema — `profiles`, `awards`, `nominations`, `finalists`, `votes`, `settings`, `allowed_domains` — with live data and a profile row created the day before. The Awards project of its own exists and is completely empty: no tables, no users. So the Awards app is running against the Product OS database, and the two share one pool of accounts. Nothing here touches those tables, because dropping them would take a live app down; moving them is a migration for the Awards repo, not a deletion.

## 2026-08-31 — The landing page starts arguing for itself (32 skills)

Niel's read on the front door was that it felt like AI slop, and he was right. It gave the whole first screen to an animated star chart, counted four numbers up on scroll, and offered four abstractions — "Signal to shipped", "Learn it once." — without once saying what the thing does or how to get it. It looked expensive. The only way in was a **Sign in** button that dead-ends at an allowlist, so a teammate who wanted the plugin had nowhere to go. Rebuilt against the reference Niel gave (Atlassian's Rovo page) and Emil Kowalski's design-engineering and animation skills.

- **The page is an argument now, and each beat earns the next.** Hero, the problem in a sentence a reader recognises, the lifecycle they can poke at, five named skills, what keeps it honest, the three setup steps, the door. The old page had one idea and repeated it.
- **Installing is the call to action, signing in is the alternative.** That is the reverse of what the page used to do. Signing in gets you the catalogue; installing gets you the thing, and "onboard the team" is the next stage. The header carries both, at all times, on every page.
- **The lifecycle is the proof, and it is specific.** Thirteen named stages, six real paths through them, and each stage's purpose on tap — all read from the same map the team uses. Picking a flow lights the stages it crosses in the order it reaches them, and says out loud where it **returns upstream**, which is the one thing a picture of a pipeline can never show.
- **Five skills are named, with the command you type.** You cannot be concrete about something you refuse to name, and refusing to name anything was the root of the vagueness. The other twenty-seven, and every trigger, stay behind the door.
- **The star chart is atmosphere now, not the argument.** It is slower, fainter and carries no labels at all, sitting behind the headline rather than in front of it.
- **The numbers are stated rather than performed.** Counting four figures up on scroll is the single most recognisable tic of a generated landing page, and it made four true facts look like decoration.
- **All 32 skill names were leaking into the public page, and had been for a month.** Star nodes carried `node.id` to the browser purely as a React key, and a node id is the skill's name with hyphens in it — `behavioural-alchemist`, `leadership-subcommittee-review` — so the page source published every one while the rendered page showed none. Reviewing the component would never have found it. Stars are keyed by an opaque index now, and `npm run check:public` fetches the live page and fails on any gated skill, trigger, system or flow step, because the only test that would have caught this is one that reads the rendered HTML.
- **Motion held to a real standard**, from Emil Kowalski's rules: strong custom easing curves (`--ease-out`, `--ease-in-out`, now app-level tokens), never `ease-in`, transform and opacity only, transitions rather than keyframes wherever a thing can be re-triggered, staggered reveals that snap back rather than staggering out, hover motion gated behind a real pointer, and press feedback on every pressable surface. `transition: all` is gone from the shared button, and the reduced-motion preference is read as external state rather than set from an effect, so a visitor who asked for less motion never sees the first frame of it.
- Fixed on review: install commands truncated mid-URL in a narrow column, which asks someone to paste a command into their shell they were never shown the end of; a manual line break in the headline that only made sense on a wide screen; and star halos that read as smudges over the type.
- **The sign-in page stops undoing the landing page.** Half the front door's calls to action point at it, and it described Flightdeck — a morning dashboard — to somebody who had just been told about thirty-two skills and a lifecycle map, so the door did not match the room it was said to open. It now says what is actually behind it, and it stops being a dead end: "ask Niel" was the only instruction offered to anyone not on the access list, which today is nearly everyone, when signing in is not how you get the Product OS at all. Installing is, and installing needs no allowlist. The denied message says so too. The header also stops offering **Sign in** while you are standing on the sign-in page.

- **The link preview is a real card now.** The way an internal tool actually spreads is somebody pasting the URL into a Slack channel, and until now that unfurled as a bare title — a wasted first impression on the exact surface the onboarding push depends on. The card is generated from the same data as the page, so its counts cannot drift, and it carries nothing the landing page would not already show. Two things worth knowing for anyone touching it: without `metadataBase` Next emits a relative `og:image`, which every unfurler silently drops, so the card would look right in the repo and be missing in Slack; and the image renderer mis-measures Space Grotesk's `tt` ligature, which put a visible hole in "written down" while the identical string was perfect in the browser.
