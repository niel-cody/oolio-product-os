...
4. **One approval for the batch**, then create the native Insights with the helper: `node ${CLAUDE_PLUGIN_ROOT}/skills/jpd-loop/scripts/jpd-insight.mjs get <KEY>` on each target first to skip duplicates, then `create --file` with one object per idea so each carries its own tailored description. Usage and fallbacks in `${CLAUDE_PLUGIN_ROOT}/skills/jpd-loop/references/jpd-insights-api.md`.
